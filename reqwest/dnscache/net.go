...
func DialFunc(baseDialFunc dialFunc) dialFunc {
	once.Lock()
	if resolverSingle == nil {
		resolverSingle = New(0, 0)
	}
	once.Unlock()

	return DialFuncWithResolver(resolverSingle, baseDialFunc)
}

// Dialer exports DialContext
type Dialer struct {
	dialFunc dialFunc
}

func NewDialer(baseDialFunc dialFunc) *Dialer {
	return &Dialer{
		dialFunc: DialFunc(baseDialFunc),
	}
}
...
// DialFuncWithResolver is a helper function which returns `net.DialContext` function.
...
func DialFuncWithResolver(resolver *Resolver, baseDialFunc dialFunc) dialFunc {
	if baseDialFunc == nil {
